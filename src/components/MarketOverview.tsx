    ...
    <div className="bg-slate-panel border border-slate-border p-6 rounded-2xl relative overflow-hidden group h-full">
      ...
      <div className="flex items-end gap-4 mb-6">
        <Tooltip content="Confidence score of the current market mode detection.">
          <div className="text-5xl font-display font-bold text-white cursor-help">{data.confidence}%</div>
        </Tooltip>
            ...
            <div className="flex justify-between items-center p-3 bg-black/20 rounded-xl border border-white/5 hover:border-primary/30 transition-all group/sig">
              ...
              <div className="text-right">
                <div className="flex items-center justify-end gap-2">
                  <span className="text-sm font-bold font-mono text-white">{sig.value}</span>
        ...
        {payload.map((entry: any, index: number) => {
          ...
          return (
            <li key={`item-${index}`} className="flex items-center gap-1.5">
              ...
              <span className="text-slate-400 uppercase">{entry.value}</span>
            ...
            {['1H', '4H', '24H', '7D'].map(t => (
              <button 
                ...
                className={cn(
                  ...
                  timeframe === t ? "bg-primary border-primary text-white" : "border-slate-border text-slate-500 hover:text-slate-300"
                )}
              >
                {t}
              </button>
            ...
            {(['btc', 'sol', 'total2'] as const).map(line => (
              <button 
                ...
                className={cn(
                  ...
                  visibleLines[line] ? "bg-white/10 border-white/20 text-white" : "border-slate-border text-slate-600"
                )}
              >
          ...
          <div className="flex gap-2">
            {(['macd', 'rsi', 'volume'] as const).map(ind => (
              <button 
                ...
                className={cn(
                  "px-2 py-1 text-[9px] font-bold rounded border transition-all uppercase",
                  indicators[ind] ? "bg-white/10 border-white/20 text-white" : "border-slate-border text-slate-600"
                )}
              >
                {ind}
              </button>
              ...
              <RechartsTooltip 
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-[#1c2632] border border-[#2d3a4b] p-3 rounded-lg shadow-xl min-w-[150px]">
                          ...
                          {payload.map((entry: any, index: number) => (
                            <div key={index} className="flex justify-between items-center gap-4">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] text-slate-300 uppercase">{entry.name}</span>
                              </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-4">
        <div className="p-3 bg-black/20 rounded-xl border border-white/5 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono text-slate-300">Global Indices</div>
          </div>
        </div>
        <div className="p-3 bg-black/20 rounded-xl border border-white/5 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono text-slate-300">SOL Ecosystem</div>
          </div>
        </div>
        <div className="p-3 bg-black/20 rounded-xl border border-white/5 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] font-mono text-slate-300">TVL Analytics</div>
          </div>
        </div>
      </div>
    </div>
            ...
            <RechartsTooltip 
              ...
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="bg-slate-900/95 backdrop-blur-md border border-slate-border p-4 rounded-xl shadow-2xl min-w-[240px]">
                        ...
                        {data.tokens && data.tokens.slice(0, 2).map((token, idx) => (
                          <div key={idx} className="flex justify-between items-center p-1.5 bg-black/20 rounded-md border border-white/5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[9px] font-bold text-white">{token.symbol}</span>
                            </div>
        ...
        {narratives.slice(0, 4).map((nar, i) => (
          ...
          >
            ...
            <div className="flex justify-between items-end">
              <div>
                <div className="text-[10px] font-display font-bold text-white">{nar.priceMomentum}</div>
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-white">{selectedNarrative.name} Intelligence</h4>
                  ...
                  <div key={idx} className="p-4 bg-black/40 rounded-2xl border border-white/5 hover:border-primary/30 transition-all group/token">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-bold text-white group-hover/token:text-primary transition-colors">{token.name}</div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono text-slate-500">{token.symbol}</span>
                            <span className="text-[8px] px-1.5 py-0.5 bg-white/5 rounded text-slate-400 uppercase font-bold">{token.source}</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-mono font-bold text-white">${token.priceUsd.toFixed(4)}</div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                      <div className="space-y-2">
                        <div className="flex justify-between text-[9px]">
                          ...
                          <span className="text-white font-mono">${(token.marketCap / 1000000).toFixed(1)}M</span>
                        </div>
                        <div className="flex justify-between text-[9px]">
                          <span className="text-slate-500 uppercase font-bold">Liquidity</span>
                          <span className="text-white font-mono">${(token.liquidity / 1000).toFixed(1)}K</span>
                        </div>
                      </div>
                      <div className="space-y-2">
                        <div className="flex justify-between text-[9px]">
                        ...
                        <div className="flex justify-between text-[9px]">
                          ...
                          <span className="text-white font-mono uppercase">{token.source}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <a 
                      ...
                      className="mt-4 w-full py-2 bg-white/5 hover:bg-primary/20 border border-white/10 hover:border-primary/30 rounded-xl text-[10px] font-bold text-slate-400 hover:text-white transition-all flex items-center justify-center gap-2"
                    >
        ...
        {data.map((rot, i) => (
          <div key={i} className="relative p-4 bg-black/20 rounded-2xl border border-white/5 overflow-hidden">
            <div className="flex justify-between items-center mb-4 relative z-10">
              <div className="flex items-center gap-3">
                <div className="px-3 py-1 bg-slate-800 rounded-lg text-xs font-bold text-slate-400 border border-white/5">
                  {rot.from}
                </div>
              ...
              <div className="text-right">
                <div className="text-lg font-display font-bold text-white">{rot.strength}%</div>
    ...
    <div className="bg-slate-panel border border-slate-border p-6 rounded-2xl group h-full">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6 mt-4">
        <div className="p-4 bg-black/20 rounded-2xl border border-white/5">
          ...
          <div className="text-2xl font-display font-bold text-white">{data.launchesPerHour} <span className="text-xs text-slate-500">/ hr</span></div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center p-3 bg-black/20 rounded-xl border border-white/5">
          <span className="text-xs text-slate-400">Avg Survival Time</span>
          <span className="text-sm font-bold font-mono text-white">{data.avgSurvivalTime}</span>
        </div>
        <div className="flex justify-between items-center p-3 bg-black/20 rounded-xl border border-white/5">
          <span className="text-xs text-slate-400">Median Market Cap</span>
          <span className="text-sm font-bold font-mono text-white">${(data.medianMarketCap / 1000).toFixed(1)}K</span>
        </div>
        <div className="flex justify-between items-center p-3 bg-black/20 rounded-xl border border-white/5">
          <span className="text-xs text-slate-400">Median Liquidity</span>
          <span className="text-sm font-bold font-mono text-white">${(data.medianLiquidity / 1000).toFixed(1)}K</span>
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex flex-col items-end gap-1">
          <div className="flex gap-1 bg-black/40 p-1 rounded-lg border border-white/5">
            <button 
              ...
              className={cn("px-2 py-1 text-[9px] font-bold rounded transition-all", view === 'flow' ? "bg-primary text-white" : "text-slate-500 hover:text-slate-300")}
            >
              FLOW
            </button>
            <button 
              onClick={() => setView('transactions')}
              className={cn("px-2 py-1 text-[9px] font-bold rounded transition-all", view === 'transactions' ? "bg-primary text-white" : "text-slate-500 hover:text-slate-300")}
            >
              TXS
            </button>
          </div>
              ...
              <div key={i} className="p-3 bg-black/20 rounded-xl border border-white/5 hover:border-white/10 transition-all">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">{tx.symbol}</span>
                  </div>
                ...
                <div className="flex justify-between items-end">
                  <div>
                    <div className="text-[10px] font-mono text-slate-400">${tx.amount.toLocaleString()}</div>
    ...
    <div className="bg-slate-panel border border-slate-border rounded-2xl overflow-hidden group flex flex-col h-full">
      <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/20">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white uppercase tracking-widest">Real-Time Market Alert Engine</h3>
        </div>
        <div className="flex items-center gap-2">
              ...
              <div key={alert.id} className="p-4 hover:bg-white/5 transition-all group/alert relative">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-white">{alert.token?.name || 'Unknown Token'}</span>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[8px] font-bold text-white">{Math.round(alert.confidence * 100)}%</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="p-1.5 bg-white/5 hover:bg-primary/20 border border-white/10 hover:border-primary/30 rounded-lg text-slate-400 hover:text-white transition-all flex items-center gap-1.5 text-[9px] font-bold"
                    >
      ...
      <div className="flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-display font-bold text-white">Global Intelligence Terminal</h1>
          </div>
